...
function confirmQuickRoll() {
  ...
  // Clear the name input and hide the form
  document.getElementById("quick-roll-name").value = "";
  document.getElementById("create-quick-roll").style.display = "none";

  // Refresh the display to show the new quick roll
  displayQuickRolls();
}
...
function triggerQuickRoll(index) {
  // Get the saved rolls and find the one at this index
  let savedRolls = localStorage.getItem("quickRolls");
  let rollsArray = JSON.parse(savedRolls);
  let roll = rollsArray[index];

  // Set all the form values to match this saved roll
  ...
  // Automatically roll with these settings
  rollSkill();
}
...
function createCharacterObject() {
  ...
  // Get quick rolls from localStorage to include in the save file
  let savedRolls = localStorage.getItem("quickRolls");
...
function loadCharacterData() {
  ...
  if (savedData) {
    ...
    // Load quick rolls if they exist in the saved data
...
function getFileInput() {
  ...
  reader.onload = function (e) {
    ...
    // Load quick rolls if they exist in the imported file
...
// This function changes selects snag automatically if the skill is untrained
function skillSelected() {
  // Identify the right skill based on the dropdown value
  let skillSelector = document.getElementById("skills").value + "-level";
  // Now see if you can get the skill level value
  let skillLevelValue = document.getElementById(skillSelector).value;
  // Now set a condition that says if the value of skillLevelValue is 0, check the snag radio button, otherwise check the "no" radio button
...
// This function rolls two D20s and returns the result depending on if the roll is with edge, snag, or neither.
function rollD20() {
  // First, check for edge or snag
...
// This function rolls the skill check depending on skill level, upshift or downshift, and specialization
function rollSkillCheck() {
  // First, find the right skill and get the skill level
  ...
  // Second, check for upshift or downshift and produce an effectiveLevel value
  ...
  // Last, check for specialization and roll the results
  ...
  if (specializationCheck.checked) {
    ...
    let displayArray = [];
    for (let i = 1; i <= effectiveLevel; i++) {
      // First, check if the roll is greater than 2, highest, AND critical
      if (
        ...
        // Second, check if the roll is highest but not critical
      } else if (specializedRolls[i - 1] === skillDieResult) {
        let dieResult = skillDice[i - 1] + ": " + specializedRolls[i - 1];
        displayArray.push(dieResult);
        console.log("Highest roll: ", dieResult);
        // Third, check if the roll is a critical (must be greater than 2 to be critical) but not highest
      } else if (
        specializedRolls[i - 1] > 2 &&
        specializedRolls[i - 1] === i * 2
      ) {
...
// This is the function triggered by the roll-button button element
function rollSkill() {
  // first get the object data from the rolling functions
  let d20Data = rollD20();
  ...
  if (
    ...
    d20Data.modifier === "with snag" &&
    skillCheckData.modifier === "with specialization"
  ) {
    resultDisplayString += " D" + skillCheckData.level * 2;
    resultDisplayString += " with Snag";
    resultDisplayString += " and Specialization</h3>";
  } else if (d20Data.modifier === "with edge" && skillCheckData.level === 0) {
    resultDisplayString += " with Edge</h3>";
  } else if (d20Data.modifier === "with snag" && skillCheckData.level === 0) {
    resultDisplayString += " with Snag</h3>";
  } else if (d20Data.modifier === "with edge") {
    resultDisplayString += " D" + skillCheckData.level * 2;
    resultDisplayString += " with Edge</h3>";
  } else if (d20Data.modifier === "with snag") {
    resultDisplayString += " D" + skillCheckData.level * 2;
    resultDisplayString += " with Snag</h3>";
  ...
  // From here, create different configurations for the D20 rolls
  resultDisplayString += "<p><strong>D20 Result: </strong>";
  if (d20Data.modifier === "with edge") {
  ...
  // From here, create the different configurations for skill die rolls starting with specialization
    ...
    for (let roll of skillCheckData.rolls) {
      ...
      if (roll === skillCheckData.result && roll > 2 && roll === critValue) {
        resultDisplayString +=
          ...
          roll +
          "</span> ";
      } // Check if the roll is critical but not highest
        ...
        resultDisplayString +=
          ...
          roll +
          "</span>";
      } // Check if the roll is highest but not critical
  ...
  // Now display the additional modifier if any
...
// First, let's get the data to save
function createCharacterObject() {
  let characterName = document.getElementById("character-name").value;
  let chosenTheme = document.getElementById("theme-selector").value;
  let skillsData = [];
...
// This function saves the character data to local storage
...
// This function loads the character data from local storage
function loadCharacterData() {
  let savedData = localStorage.getItem("rangerCharacterData");
  if (savedData) {
    let parsedData = JSON.parse(savedData);
...
// Now an actual file to download the character data
...
// Last, set up file upload to load character data
function getFileInput() {
  let fileInput = document.getElementById("import-skills-file");
  let file = fileInput.files[0];
  if (!file) {
...
// Custom dice
// Step 1: HTML structure for custom dice input
...
// Step 2: Set up custom dice pool logic
...
// Saving roll configurations
// When the user presses the "Save roll configuration" button:
// 1.
